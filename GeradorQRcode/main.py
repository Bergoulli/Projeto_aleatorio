...
def generate():
    ...
    text = entry.get()
    qr=qrcode.make(text)
    qr.save('qrcodes/'+str(name)+'.png') #usar o titulo para nomear os qrcodes
    global Image
    Image=PhotoImage(file='qrcodes/'+str(name)+'.png') #apenas para gerar o qrcode na tela
...
Button(root,text='Gerador', width=20, height=2, bg='black', fg='white', command=generate).place(x=50,y=300)
